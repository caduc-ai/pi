...
- Exported `ENV_AGENT_DIR` and `APP_NAME` from the package root, alongside the existing `getAgentDir`/`CONFIG_DIR_NAME` exports, so callers (e.g. `@earendil-works/pi-server`'s account namespaces) can override a spawned session's agent config directory via environment variable without hardcoding the `PI_`-prefixed name.
- Added model-generated session titles: after the first turn settles, a session with no explicit name gets a short (3-6 word) title generated by a background, non-blocking completion using the session's current model. Failures are logged, not surfaced; a session that already has a name is never renamed. Disable with the new `autoNameSession: false` setting.
- Added workflow slash commands to the subagent example extension: `/implement`, `/scout-and-plan`, and `/implement-and-review`.
- Added web mode: `pi --web [--web-port <port>] [--web-host <host>]` serves the pi web UI over HTTP/WebSocket with a per-run auth token, giving full session control from a browser. See [rpc.md](docs/rpc.md).
- Added `/web [off|<host>]` command to share a live interactive session to the pi web UI mid-flight, displaying a URL and QR code. Extension dialogs are multiplexed between the TUI and connected web clients (first response wins).
- Added read-only session viewer: `pi --web --view <session.jsonl>` serves a recorded session file to the web UI without an agent process.
- Added a `/themes` endpoint to web mode listing available themes, served like the TUI resolves them.
- Added builtin slash commands to the RPC protocol: `get_commands` now also returns `/compact`, `/new`, `/name`, `/model`, `/session`, `/export`, `/copy`, `/fork`, and `/clone` with source `"builtin"`, mapped to RPC command verbs.
- Added `/cd <path>` to change a session's working location mid-flight, in the TUI and over RPC (new `change_cwd` command; `get_state` now reports `cwd`). The session history is forked into a new session file under the target directory, and all cwd-bound resources are reloaded for the new directory.
- Added a persistent terminal to the RPC protocol: `terminal_open`, `terminal_input`, `terminal_resize`, and `terminal_close` commands plus `terminal_output` and `terminal_exit` events. It is backed by a tmux session and requires `tmux` on the host. See [rpc.md](docs/rpc.md#terminal).
- Added PWA manifest MIME support for `pi --web`, allowing the web UI's install metadata to be served correctly.
- Added a TUI attach mode to the RPC protocol: `tui_open`, `tui_input`, `tui_resize`, and `tui_close` commands plus `tui_output` and `tui_exit` events, letting a client attach the real pi interactive TUI to its own session over a tmux-backed pty. See [rpc.md](docs/rpc.md#tui).
...
- Fixed model-generated session titles being regenerated every time a new `AgentSession` was created for the same session file by persisting the naming attempt to the session file synchronously before the title request starts.
- Fixed RPC clients not seeing changes the TUI made until the TUI was closed: the bridge now watches the session file while a TUI is attached and reloads it (debounced), broadcasting a new `session_reloaded` event.
- Fixed the `/theme/<name>.json` endpoint in `pi --web`'s standalone web server always returning 404.
- Fixed the TUI-attached write guard only blocking `prompt`/`steer`/`follow_up`: other session-mutating commands are now also rejected with `"TUI is attached to this session"` while a TUI is attached.
- Fixed `tui_open` racing two concurrent calls into spawning two TUI processes attached to the same session file.
- Fixed `tui_close` only notifying the closing client: every other client attached to the same session now also receives `tui_exit`.
- Fixed the web/RPC terminal and TUI failing to find `tmux` when it is only installed via a non-default prefix: tmux discovery now falls back to a `PATH` search.
- Fixed leaked `pi-<pid>-*` tmux sessions from a crashed (SIGKILLed) prior `pi` process never being cleaned up: interactive, web, and RPC mode startup now reaps stale sessions.
