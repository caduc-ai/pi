...
- Added model-generated session titles: after the first turn settles (session has both a user and an assistant message), a session with no explicit name gets a short (3-6 word) title generated by a background, non-blocking completion using the session's current model, built from the first user message and assistant reply. Failures are logged, not surfaced; a session that already has a name (set explicitly or generated earlier) is never renamed, and the check runs at most once per session per process. Disable with the new `autoNameSession: false` setting.
...
- Fixed model-generated session titles being regenerated (and overwritten with a different title) every time a new `AgentSession` was created for the same session file - resume, a pinned session's auto-respawn, a server restart, or a session reload (`switchSession` back onto the same file while a TUI is attached). The per-instance "already tried" flag reset with each new instance, but the generation itself is a slow, non-blocking background request; if a reload created a new instance while an earlier instance's request was still in flight, the new instance had no way to see that naming had already started and ran its own independent generation, appending a second (usually different) title. Fixed by persisting the naming attempt to the session file synchronously, before the request starts (a reservation `session_info` entry), so any process that (re)loads the session afterward sees the attempt and never retries; the invariant is now "auto-named at most once ever," and an in-flight request is also aborted if its `AgentSession` is disposed before it completes. A user rename is still never overwritten.
