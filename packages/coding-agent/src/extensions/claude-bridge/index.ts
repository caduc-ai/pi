...
import { compact } from "../../core/compaction/index.ts";
import type { ExtensionAPI, ExtensionUIContext } from "../../core/extensions/types.ts";
import { buildSessionContext, type CompactionEntry } from "../../core/session-manager.ts";
...
type ClaudeBridgeContentBlockParam =
	| { type: "text"; text: string }
	| { type: "image"; source: { type: "base64"; media_type: string; data: string } };

type ClaudeBridgeMessageParam = { role: "user"; content: ClaudeBridgeContentBlockParam[] };
...
function proxyProtocol(value: string | undefined): string | undefined {
	const proxy = value?.trim();
	if (!proxy) return undefined;
	try {
		return new URL(proxy).protocol;
	} catch {
		return undefined;
	}
}
...
function buildClaudeCodeEnv(overrides: Record<string, string>): NodeJS.ProcessEnv {
	const env: NodeJS.ProcessEnv = { ...process.env, ...overrides };
	const configuredProxy = providerSettings.httpProxy?.trim() || env.CLAUDE_BRIDGE_HTTP_PROXY?.trim();
	if (configuredProxy) {
		if (!isHttpProxy(configuredProxy)) {
			throw new Error(
				`Claude Code only supports HTTP(S) proxy URLs in HTTP_PROXY/HTTPS_PROXY; got ${configuredProxy}. ` +
					"Put an HTTP CONNECT proxy in front of SOCKS5 and set provider.httpProxy or CLAUDE_BRIDGE_HTTP_PROXY to that URL.",
			);
		}
	...
	const inheritedSocks = CLAUDE_PROXY_ENV_VARS.find((name) => isSocksProxy(env[name]));
	if (inheritedSocks) {
		throw new Error(
			`Claude Code cannot use inherited ${inheritedSocks}=${env[inheritedSocks]} because its runtime rejects SOCKS proxies. ` +
				"Use vpnproxy's HTTP bridge (for example http://127.0.0.1:1081) and set pi httpProxy/provider.httpProxy to that HTTP URL.",
		);
	}
	return env;
}
...
function extractUserPromptBlocks(messages: Context["messages"]): ClaudeBridgeContentBlockParam[] | null {
	const last = messages[messages.length - 1];
	if (!last || last.role !== "user") return null;
	...
	let hasImage = false;
	const blocks: ClaudeBridgeContentBlockParam[] = [];
	for (const block of last.content) {
		if (block.type === "text" && block.text) {
			...
			blocks.push({
				type: "image",
				source: { type: "base64", media_type: block.mimeType as string, data: block.data },
			});
		}
	}
...
async function* wrapPromptStream(blocks: ClaudeBridgeContentBlockParam[]): AsyncIterable<SDKUserMessage> {
	yield {
		type: "user",
		message: { role: "user", content: blocks } as ClaudeBridgeMessageParam as SDKUserMessage["message"],
...
async function runIsolatedSummary(
	...
	try {
		...
		sdkQuery = query({
			...
			options: {
				cwd,
				env: buildClaudeCodeEnv({ DISABLE_AUTO_COMPACT: "1", CLAUDE_CODE_DISABLE_AUTO_MEMORY: "1" }),
				tools: [],
...
function streamClaudeAgentSdk(
	...
	const childEnv = buildClaudeCodeEnv({ ENABLE_CLAUDEAI_MCP_SERVERS: "0", DISABLE_AUTO_COMPACT: "1" });
...
async function promptAndWait(
	...
	const sdkQuery = query({
		...
		options: {
			cwd,
			env: buildClaudeCodeEnv({ ENABLE_CLAUDEAI_MCP_SERVERS: "0", DISABLE_AUTO_COMPACT: "1" }),
			permissionMode: "bypassPermissions",
