...
- Changed the dashboard and settings pages to read the same theme choice the web UI's topbar toggle persists (one `localStorage` key, set by an inline script before first paint) instead of only following the OS's `prefers-color-scheme`, so switching the app's theme now also matches these pages; still falls back to `prefers-color-scheme` when nothing is stored yet.
